        ...
        for n_cities, destroy_cnt in test_cases:
            ...
            if op_type == 'destroy':
                ...
                if len(combined) != n_cities:
                    return_dict['result'] = (False, f"Lost/duplicated cities. Unique: {len(combined)}")
                    return
                ...
                try:
                    new_sol = target_func(new_temp_sol, to_remove_values, problem_data)
                except TypeError:
                    new_sol = target_func(new_temp_sol, to_remove_values)
...
class CVRPTask:
    ...
    def _load_instances(self):
        ...
        for i, (coords, dist_mat, demands, cap) in enumerate(raw_instances):
            ...
            instance = CVRPInstance(
                ...
                demands=demands,
                capacity=cap,
                name=f"CVRP_Instance_{i}"
            )
            self.instances.append(instance)

    @property
    def current_instance(self) -> CVRPInstance:
    ...
    def evaluate(self, solution: List[List[int]]) -> float:
        ...
        for route in solution:
            ...
            total_dist += dist_mat[route[-1]][depot]
    ...
    def get_initial_operators(self):
        seeds_destroy = [
            ...
            {'name': 'center_destroy', 'func': destroy_v3, 'code': inspect.getsource(destroy_v3), 'score': 0.0, 'weight': 1.0}
        ]
        seeds_repair = [
            ...
            {'name': 'demand_sorted_greedy_insert', 'func': insert_v3, 'code': inspect.getsource(insert_v3), 'score': 0.0, 'weight': 1.0}
        ]
